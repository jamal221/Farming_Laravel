...
<div class="card-header" id="all"style="width: 200%">
        {{-- @dd($fetched_farms) --}}
            ...
            <div class="card card-primary">
                <h1 style="padding-left: 30px">You are adding NetPipe analyze result to this farm</h1>
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            <label for="NetPipeNum">Number of NetPipes for this Farm:</label>
                            <input type="text" readonly="true" class="form-control" id="NetPipeNum" name="NetPipeNum" value="{{$fetched_NetPipes_count}}">
                        </div>
...
@if($fetched_NetPipes_count)
@php
    $count=1;
    $div_width=100/(int)$fetched_NetPipes_count;
@endphp
   
<div style="width: 100%">       
    @foreach($fetched_NetPipes as $key=>$value)
       
        
                {{-- @dd($fetched_farms) --}}
                <div class="card-header" id="NetPipesAll" style="width:{{$div_width}}%">
                    <div class="card card-primary" >
                        <div class="card-body" >
                                        <div class="form-group" >
                                            <label for="NetPipes{{$value->id}}">NetPipes : {{$count++}}</label>
                                            @if(in_array($value->id, $fetched_NetPipes_Analyzed))
                                                <button type="button" name="NetPipes{{$value->id}}" id="NetPipes{{$value->id}}" onclick="updateValues({{$value->id}}, {{$fetched_farms[0]->FarmID}})" class="btn btn-success btn-xs show_analyze_table"  >Show Table to Update</button>
                                            @else
                                                <button type="button" name="NetPipes{{$value->id}}" id="NetPipes{{$value->id}}" onclick="regValues({{$value->id}}, {{$fetched_farms[0]->FarmID}})" class="btn btn-info btn-xs show_analyze_table"  >Show Table to Register</button>
                                            @endif
                                        </div>
                        </div>  
                    </div>  
...
@else
    <h1 style="color: red">There is no NetPipes to register analyze</h1>
...
<script type="text/javascript">
    ...
    var _token = $('input[name="_token"]').val();
        function regValues(NetPipeID,FarmID){
            Resource_ID_page=NetPipeID;
            FarmID_page=FarmID;
        ...
        $(document).on('click', '.add_water_analyze', function(){
            if(!confirm("Did you want to Register the detials of this Analyze to this NetPipe?")) {
                  return false;
              };
            ...
            console.log({
                ...
                "Token":_token
                                    ...
                                    $.ajax({    
                                        url:"{{ route('RegAnalyzeWaterNetPipes') }}",
                                        method:"POST",
                                        data:{
    ...
    function updateValues(NetPipeID, FarmID){
        Resource_ID_page=NetPipeID;
        FarmID_page=FarmID;
        console.log({
            "NetPipe_ID":NetPipeID,
            "Farm_ID":FarmID
        })
                                 $.ajax({    
                                        url:"{{ route('AnalyzedNetPipesData') }}",
                                        method:"POST",
                                        data:{
                                             Resource_ID:NetPipeID,
                                             farm_ID:FarmID,
                                            _token:_token
                                        },
                                        success: function(data){ // What to do if we succeed
