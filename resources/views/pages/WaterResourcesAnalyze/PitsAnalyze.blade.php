...
<div class="card-header" id="all"style="width: 200%">
        {{-- @dd($fetched_farms) --}}
            ...
            <div class="card card-primary">
                <h1 style="padding-left: 30px">You are adding Pit analyze result to this farm</h1>
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            <label for="pitNum">Number of Pits for this Farm:</label>
                            <input type="text" readonly="true" class="form-control" id="pitNum" name="pitNum" value="{{$fetched_pits_count}}">
                        </div>
...
@if($fetched_pits_count)
@php
    $count=1;
    $div_width=100/(int)$fetched_pits_count;
@endphp
...
<div style="width: 100%">       
    @foreach($fetched_pits as $key=>$value)
                ...
                {{-- @dd($fetched_farms) --}}
                <div class="card-header" id="PitsAll" style="width:{{$div_width}}%">
                    <div class="card card-primary" >
                        <div class="card-body" >
                                        <div class="form-group" >
                                            <label for="Pits{{$value->id}}">Pits : {{$count++}}</label>
                                            @if(in_array($value->id, $fetched_pits_Analyzed))
                                                <button type="button" name="Pits{{$value->id}}" id="Pits{{$value->id}}" onclick="updateValues({{$value->id}}, {{$fetched_farms[0]->FarmID}})" class="btn btn-success btn-xs show_analyze_table"  >Show Table to Update</button>
                                            @else
                                                <button type="button" name="Pits{{$value->id}}" id="Pits{{$value->id}}" onclick="regValues({{$value->id}}, {{$fetched_farms[0]->FarmID}})" class="btn btn-info btn-xs show_analyze_table"  >Show Table to Register</button>
                                            @endif
                                        </div>
                        </div>  
                    </div>  
...
@else
    <h1 style="color: red">There is no Pits to register analyze</h1>
...
<script type="text/javascript">
    ...
    var _token = $('input[name="_token"]').val();
        function regValues(pitID,FarmID){
            Pet_ID_page=pitID;
            FarmID_page=FarmID;
        ...
        $(document).on('click', '.add_water_analyze', function(){
            if(!confirm("Did you want to Register the detials of this Analyze to this Pit?")) {
                  return false;
              };
    ...
    function updateValues(pitID, FarmID){
        Pet_ID_page=pitID;
        FarmID_page=FarmID;
        console.log({
            "pit_ID":pitID,
