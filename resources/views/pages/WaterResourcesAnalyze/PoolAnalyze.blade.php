...
<div class="card-header" id="all"style="width: 200%">
        {{-- @dd($fetched_farms) --}}
            ...
            <div class="card card-primary">
                <h1 style="padding-left: 30px">You are adding Pool analyze result to this farm</h1>
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            <label for="PoolNum">Number of Pools for this Farm:</label>
                            <input type="text" readonly="true" class="form-control" id="PoolNum" name="PoolNum" value="{{$fetched_Pools_count}}">
                        </div>
...
@if($fetched_Pools_count)
@php
    $count=1;
    $div_width=100/(int)$fetched_Pools_count;
@endphp
...
<div style="width: 100%">       
    @foreach($fetched_Pools as $key=>$value)
                ...
                {{-- @dd($fetched_farms) --}}
                <div class="card-header" id="PoolsAll" style="width:{{$div_width}}%">
                    <div class="card card-primary" >
                        <div class="card-body" >
                                        <div class="form-group" >
                                            <label for="Pools{{$value->id}}">Pools : {{$count++}}</label>
                                            @if(in_array($value->id, $fetched_Pools_Analyzed))
                                                <button type="button" name="Pools{{$value->id}}" id="Pools{{$value->id}}" onclick="updateValues({{$value->id}}, {{$fetched_farms[0]->FarmID}})" class="btn btn-success btn-xs show_analyze_table"  >Show Table to Update</button>
                                            @else
                                                <button type="button" name="Pools{{$value->id}}" id="Pools{{$value->id}}" onclick="regValues({{$value->id}}, {{$fetched_farms[0]->FarmID}})" class="btn btn-info btn-xs show_analyze_table"  >Show Table to Register</button>
                                            @endif
                                        </div>
                        </div>  
                    </div>  
...
@else
    <h1 style="color: red">There is no Pools to register analyze</h1>
...
<script type="text/javascript">
    ...
    var _token = $('input[name="_token"]').val();
        function regValues(PoolID,FarmID){
            Resource_ID_page=PoolID;
            FarmID_page=FarmID;
        ...
        $(document).on('click', '.add_water_analyze', function(){
            if(!confirm("Did you want to Register the detials of this Analyze to this Pool?")) {
                  return false;
              };
            ...
            console.log({
                ...
                "Token":_token
                                    ...
                                    $.ajax({    
                                        url:"{{ route('RegAnalyzeWaterPools') }}",
                                        method:"POST",
                                        data:{
    ...
    function updateValues(PoolID, FarmID){
        Resource_ID_page=PoolID;
        FarmID_page=FarmID;
        console.log({
            "Pool_ID":PoolID,
