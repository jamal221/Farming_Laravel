...
<div class="card-header" id="all"style="width: 200%">
        {{-- @dd($fetched_farms) --}}
            ...
            <div class="card card-primary">
                <h1 style="padding-left: 30px">You are adding River analyze result to this farm</h1>
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            <label for="RiverNum">Number of Rivers for this Farm:</label>
                            <input type="text" readonly="true" class="form-control" id="RiverNum" name="RiverNum" value="{{$fetched_Rivers_count}}">
                        </div>
...
@if($fetched_Rivers_count)
@php
    $count=1;
    $div_width=100/(int)$fetched_Rivers_count;
@endphp
   
<div style="width: 100%">       
    @foreach($fetched_Rivers as $key=>$value)
       
        
                {{-- @dd($fetched_farms) --}}
                <div class="card-header" id="RiversAll" style="width:{{$div_width}}%">
                    <div class="card card-primary" >
                        <div class="card-body" >
                                        <div class="form-group" >
                                            <label for="Rivers{{$value->id}}">Rivers : {{$count++}}</label>
                                            @if(in_array($value->id, $fetched_Rivers_Analyzed))
                                                <button type="button" name="Rivers{{$value->id}}" id="Rivers{{$value->id}}" onclick="updateValues({{$value->id}}, {{$fetched_farms[0]->FarmID}})" class="btn btn-success btn-xs show_analyze_table"  >Show Table to Update</button>
                                            @else
                                                <button type="button" name="Rivers{{$value->id}}" id="Rivers{{$value->id}}" onclick="regValues({{$value->id}}, {{$fetched_farms[0]->FarmID}})" class="btn btn-info btn-xs show_analyze_table"  >Show Table to Register</button>
                                            @endif
                                        </div>
                        </div>  
                    </div>  
...
@else
    <h1 style="color: red">There is no Rivers to register analyze</h1>
...
<script type="text/javascript">
    ...
    var _token = $('input[name="_token"]').val();
        function regValues(RiverID,FarmID){
            Resource_ID_page=RiverID;
            FarmID_page=FarmID;
        ...
        $(document).on('click', '.add_water_analyze', function(){
            if(!confirm("Did you want to Register the detials of this Analyze to this River?")) {
                  return false;
              };
            ...
            console.log({
                ...
                "Token":_token
                                    ...
                                    $.ajax({    
                                        url:"{{ route('RegAnalyzeWaterRivers') }}",
                                        method:"POST",
                                        data:{
    ...
    function updateValues(RiverID, FarmID){
        Resource_ID_page=RiverID;
        FarmID_page=FarmID;
        console.log({
            "River_ID":RiverID,
