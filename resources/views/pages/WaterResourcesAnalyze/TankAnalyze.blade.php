...
<div class="card-header" id="all"style="width: 200%">
        {{-- @dd($fetched_farms) --}}
            ...
            <div class="card card-primary">
                <h1 style="padding-left: 30px">You are adding Tank analyze result to this farm</h1>
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            <label for="TankNum">Number of Tanks for this Farm:</label>
                            <input type="text" readonly="true" class="form-control" id="TankNum" name="TankNum" value="{{$fetched_Tanks_count}}">
                        </div>
...
@if($fetched_Tanks_count)
@php
    $count=1;
    $div_width=100/(int)$fetched_Tanks_count;
@endphp
...
<div style="width: 100%">       
    @foreach($fetched_Tanks as $key=>$value)
                ...
                {{-- @dd($fetched_farms) --}}
                <div class="card-header" id="TanksAll" style="width:{{$div_width}}%">
                    <div class="card card-primary" >
                        <div class="card-body" >
                                        <div class="form-group" >
                                            <label for="Tanks{{$value->id}}">Tanks : {{$count++}}</label>
                                            @if(in_array($value->id, $fetched_Tanks_Analyzed))
                                                <button type="button" name="Tanks{{$value->id}}" id="Tanks{{$value->id}}" onclick="updateValues({{$value->id}}, {{$fetched_farms[0]->FarmID}})" class="btn btn-success btn-xs show_analyze_table"  >Show Table to Update</button>
                                            @else
                                                <button type="button" name="Tanks{{$value->id}}" id="Tanks{{$value->id}}" onclick="regValues({{$value->id}}, {{$fetched_farms[0]->FarmID}})" class="btn btn-info btn-xs show_analyze_table"  >Show Table to Register</button>
                                            @endif
                                        </div>
                        </div>  
                    </div>  
...
@else
    <h1 style="color: red">There is no Tanks to register analyze</h1>
...
<script type="text/javascript">
    ...
    var _token = $('input[name="_token"]').val();
        function regValues(TankID,FarmID){
            Resource_ID_page=TankID;
            FarmID_page=FarmID;
        ...
        $(document).on('click', '.add_water_analyze', function(){
            if(!confirm("Did you want to Register the detials of this Analyze to this Tank?")) {
                  return false;
              };
            ...
            console.log({
                ...
                "Token":_token
                                    ...
                                    $.ajax({    
                                        url:"{{ route('RegAnalyzeWaterTanks') }}",
                                        method:"POST",
                                        data:{
    ...
    function updateValues(TankID, FarmID){
        Resource_ID_page=TankID;
        FarmID_page=FarmID;
        console.log({
            "Tank_ID":TankID,
